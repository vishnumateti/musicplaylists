...
class MusicLists extends Component {
  state = {searchInput: '', playLists: initialTracksList}
  ...
  onClickDeleteTrack = id => {
    ...
    const filteredData = playLists.filter(deleteItem => deleteItem.id !== id)

    this.setState({playLists: filteredData})
  }

  render() {
    const {playLists, searchInput} = this.state
    const searchResults = playLists.filter(eachTrack =>
    ...
    return (
      <AppContainer>
        ...
        <PlayListContainer>
          <PlayListText>Songs Playlist</PlayListText>
          <SearchInput
            type="search"
            value={searchInput}
            placeholder="Search"
